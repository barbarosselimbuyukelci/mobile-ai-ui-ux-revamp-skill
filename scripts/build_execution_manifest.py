...
def build_manifest(artifact_dir: Path, files: Dict[str, str]) -> str:
    ...
    lines.append("- Produce `15-artifact-intake.md` by reading all source artifacts.")
    lines.append("- Produce `16-execution-batch-plan.md` (delegate to planning skill/agent if available).")
    ...
    lines.append("- Run cross-artifact consistency check on run artifacts.")
    lines.append("- Run execution-readiness check before final delivery claim.")
    ...
    lines.append("- Cross-artifact consistency: required")
    lines.append("- Execution-readiness gate: required")
    lines.append("")
    ...
    lines.append("- 15-artifact-intake.md")
    lines.append("- 16-execution-batch-plan.md")
    lines.append("- 17-implementation-change-log.md")
    lines.append("- traceability-matrix.md or csv")
    lines.append("- Consistency validation output (script pass/fail)")
    lines.append("")
    ...
    lines.append("- Before delivery, run: python scripts/check_artifact_consistency.py <run-artifacts/run-id>.")
    lines.append("- Before delivery, run: python scripts/check_execution_readiness.py <run-artifacts/run-id>.")
